DEMO_REPORT = {
    ...
    "pr": {
        ...
        "commits": [{"sha": "demo", "message": "新增支付快速路径", "author": "demo-dev"}],
    },
    "summary": {
        "what_changed": "这个 PR 修改了支付授权逻辑，并新增了订单查询路径。",
        "risk_overview": "系统识别出两个高风险文件，因为变更涉及支付、权限、数据库和错误处理行为，但没有对应的测试更新。",
    ...
    "file_risks": [
        ...
        },
        {
            ...
            "risk_dimensions": ["security", "data", "test_gap"],
        },
    ],
    "findings": [
        {
            ...
            "comment_draft": "`user.role` 缺失时这里的提前返回似乎会允许支付写入。建议改成默认拒绝，并补充角色覆盖测试。",
        },
        {
            ...
            "comment_draft": "这个查询建议使用参数化方式，不要把 `id` 直接插入 SQL 字符串。",
        },
        {
            "title": "行为变更缺少对应测试更新",
            "severity": "P2",
